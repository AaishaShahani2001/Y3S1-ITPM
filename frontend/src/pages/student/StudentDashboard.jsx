...
import MyEvents from "./MyEvents"; // ✅ ONLY REQUIRED COMPONENT
    ...
    <div className="flex min-h-screen bg-slate-50 font-sans text-slate-900">
      ...
      <aside className="w-20 md:w-64 bg-slate-900 text-white flex flex-col sticky top-0 h-screen transition-all duration-300">
        <div className="p-4 md:p-8 flex items-center justify-center md:justify-start gap-3">
          <div className="w-8 h-8 bg-blue-600 rounded-lg flex items-center justify-center text-lg font-black">
            M
          </div>
          <span className="hidden md:block text-lg font-black tracking-tighter">
            MindBridge
          </span>
        </div>
        ...
        <nav className="flex-1 mt-6 px-3 space-y-1.5">
          {TABS.map((tab) => (
            <button
              ...
              className={`w-full flex items-center justify-center md:justify-start gap-3 p-3.5 rounded-xl transition-all group ${
                activeTab === tab.id
            ...
            >
              ...
              <span className="hidden md:block font-bold text-xs tracking-wide">
                {tab.label}
              </span>
            </button>
          ))}
        ...
        <div className="p-4 md:p-8 border-t border-slate-800">
          <button
            ...
            className="w-full flex items-center justify-center md:justify-start gap-3 text-slate-400 hover:text-red-400 transition-colors"
          >
            <FaSignOutAlt className="text-lg" />
            <span className="hidden md:block font-bold text-xs">
              Logout
            </span>
          </button>
        </div>
      ...
      <main className="flex-1 flex flex-col overflow-x-hidden">
        ...
        <header className="h-16 bg-white border-b border-slate-100 px-6 md:px-10 flex items-center justify-between sticky top-0 z-10">
          <div className="relative hidden md:block w-80">
            ...
            <input
              type="text"
              placeholder="Search..."
              className="w-full pl-10 pr-4 py-2 bg-slate-50 border-none rounded-lg text-xs focus:ring-2 focus:ring-blue-100 outline-none"
            />
          </div>

          <div className="flex items-center gap-5">
            <button className="relative w-8 h-8 rounded-lg bg-slate-50 flex items-center justify-center text-slate-500 hover:bg-blue-50 hover:text-blue-600">
        ...
        <div className="p-6 md:p-10 max-w-6xl w-full mx-auto">
          ...
          {activeTab === "appointments" && <p>Appointments coming soon...</p>}
